...
class Transition extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    return SlideTransition(
      ...
      child: Opacity(
        opacity: isModel ? 1.0 : driveAnimation.value.clamp(0.0, 1.0).toDouble(),
        child: child,
      ),
    );
  }
  ...
  factory Transition.container(
          {required Widget child, required Animation<double> driveAnimation}) =>
      Transition._(
        driveAnimation: driveAnimation,
        positionAnimation: Tween(begin: const Offset(0.0, 1.0), end: Offset.zero),
        child: child,
      );

  /// Creates transition animation for model in `AnimalDetailsPage`
  factory Transition.model({required Widget child, required Animation<double> driveAnimation}) =>
      Transition._(
        isModel: true,
        ...
        positionAnimation: Tween(begin: Offset.zero, end: const Offset(0, -0.25)),
  ...
  factory Transition.backButton(
      ...
      Transition._(
        driveAnimation: driveAnimation,
        positionAnimation: Tween(begin: const Offset(-1.4, 0.0), end: Offset.zero),
        child: child,
      );
...
class IndicatorAnimation extends AnimatedWidget {
  ...
  const IndicatorAnimation({required this.animation, required this.child})
...
class FadeFromUpAnimation extends StatefulWidget {
  ...
  const FadeFromUpAnimation({required this.child});
  @override
...
class _FadeFromUpAnimationState extends State<FadeFromUpAnimation>
  ...
  @override
  void initState() {
    _animationController = AnimationController(vsync: this, duration: kDuration)..forward();
    _animation = Tween<Offset>(begin: const Offset(0, 0.2), end: const Offset(0, 0))
